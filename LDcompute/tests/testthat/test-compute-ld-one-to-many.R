...
test_that("works normally with two variants", {
  ...
  chk <- snpgdsLDMat(gds, snp.id = c(1, 2, 3), slide = -1, verbose = FALSE)$LD
...
test_that("works normally with three variants", {
  ...
  expect_equal(ld$ld_composite, chk$LD[1, 2:4])
})
...
test_that("variant_include_1 is not the lowest variant id", {
  ...
  # Check against the function directly.
  chk <- snpgdsLDMat(gds, snp.id = c(1, 2, 3, 4), slide = -1, verbose = FALSE)
  expect_equal(ld$ld_composite, chk$LD[2, c(1, 3, 4)])
})
...
test_that("missing data", {
  ...
  expect_true(is.numeric(ld$ld_composite))
  expect_true(all(!is.na(ld$ld_composite)))
})
...
test_that("sample set", {
  ...
  expect_equal(ld_sub$ld_composite, chk$LD[1,2:3])
})
...
test_that("works with large variant ids", {
  ...
  # Check against the function directly.
  chk <- snpgdsLDMat(gds, snp.id = c(10000, 2, 3), slide = -1, verbose = FALSE)$LD
  expect_equal(ld$ld_composite[1], chk[3,1])

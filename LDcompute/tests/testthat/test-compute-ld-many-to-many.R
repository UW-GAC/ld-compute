...
test_that("different methods", {
  ...
  ld <- compute_ld_set(gds, var_include, methods = "composite")
  expect_equal(names(ld), c("variant.id.1", "variant.id.2", "ld_composite"))
  ...
  ld <- compute_ld_set(gds, var_include, methods = "dprime")
  ...
  ld <- compute_ld_set(gds, var_include, method = "corr")
  ...
  ld <- compute_ld_set(gds, var_include, method = "r")
  ...
  # Method not allowed.
  expect_error(compute_ld_set(gds, var_include, method = "foo"), "allowed methods")
...
test_that("multiple methods are allowed", {
  ...
  ld_composite <- compute_ld_set(gds, var_include, methods = "composite")
  ld_dprime <- compute_ld_set(gds, var_include, methods = "dprime")
  ld_corr <- compute_ld_set(gds, var_include, method = "corr")
  ld_r <- compute_ld_set(gds, var_include, method = "r")

  ld_full <- compute_ld_set(gds, var_include, methods = c("composite", "dprime", "corr", "r"))
  ...
  # Method not allowed.
  expect_error(compute_ld_set(gds, var1, var2, method = c("r", "foo")), "allowed methods")
})
...
test_that("works with large variant ids", {
  ...
  expect_equal(ld$ld_composite[1], chk[1,2])
  expect_equal(ld$ld_composite[2], chk[1,3])
  expect_equal(ld$ld_composite[3], chk[2,3])
})
